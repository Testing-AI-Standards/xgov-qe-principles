# xgov-qe-principles

![Quality Check](https://github.com/Testing-AI-Standards/xgov-qe-principles/actions/workflows/standards-check.yml/badge.svg)

![In Review](https://img.shields.io/badge/Phase-In%20Review-blue)
<!-- cspell:disable-next-line -->
## Quality Engineering Principles for Public Sector

This repository contains the  Cross-Government Testing community's QE principles.  
The goal is to gather input and feedback to refine these QE principles.

## 📄 Document

The principles are located in [principles.md](./principles.md)

## 💬 Contributing Feedback

We welcome contributions and feedback from the community. You can use one of the options:

- Open an [Issue](https://github.com/Testing-AI-Standards/xgov-qe-principles/issues) to ask a question or suggest improvements > New Issue > Feedback on principles > Fill the details
- Submit a Pull Request with proposed changes. Guidance for raising PRs is available in the [contributing guide](./CONTRIBUTING.md)
- Use [Discussion](https://github.com/orgs/Testing-AI-Standards/discussions) > Feedback - QE Principles. Tag specific sections for clarification or refinement

Thank you for contributing! 🙌

## 📄 Licence

Unless stated otherwise, the framework is released under the MIT License. This covers both the files and any sample code in the documentation.

The documentation is [© Crown copyright](https://www.nationalarchives.gov.uk/information-management/re-using-public-sector-information/uk-government-licensing-framework/crown-copyright/) and available under the terms of the [Open Government 3.0 licence](https://www.nationalarchives.gov.uk/doc/open-government-licence/version/3/).
